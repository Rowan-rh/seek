# runtime-home-isolation Specification
...
系统 SHALL 在进程启动时读取 `SEEK_HOME`。当该变量为非空路径时，项目配置、统一配置、旧版兼容配置、用户链路覆盖、报告模板覆盖、会话、错误日志、性能日志和版本标记等原位于 `~/.seek` 的内容 MUST 全部改为位于 `SEEK_HOME` 下的相同相对路径；未设置时 SHALL 继续使用 `~/.seek`。版本标记和诊断日志属于 best-effort 辅助信息，其读写失败 MUST NOT 覆盖或阻断原始命令结果。
...
#### Scenario: 版本标记不可写不阻断命令
- **WHEN** `SEEK_HOME` 不可作为目录写入，CLI 无法创建或更新版本标记
- **THEN** CLI 在 stderr 输出版本标记写入失败提示
- **AND** 继续执行原命令并保持其 stdout JSON 与退出码语义
