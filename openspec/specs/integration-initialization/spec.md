# integration-initialization Specification

## Purpose
定义 seek 首次使用或环境迁移时的外部集成初始化体验，使用户能在开始排查前发现 A1 CLI、DMS MCP 与 SLS 凭据缺口，并获得安全、可执行的配置和验证指引。

## Requirements

### Requirement: 初始化命令必须检查必要集成
系统 SHALL 提供 `seek init` 命令，检查 A1 CLI 是否安装、DMS MCP 配置是否存在且结构有效、SLS SDK 与 AK/SK 是否已配置，并以结构化结果返回每项状态。
...
默认 `seek init` SHALL 只执行本地文件和可执行程序检查。仅当用户传入 `--verify` 时，系统 SHALL 尝试验证 A1 登录态和 DMS MCP 工具发现；验证失败 SHALL 转化为结构化状态和修复建议，不得导致未捕获异常。
